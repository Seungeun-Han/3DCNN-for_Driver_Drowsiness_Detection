...
mirrored_strategy = tf.distribute.MirroredStrategy(devices=["/GPU:3", "/GPU:4", "/GPU:5", "/GPU:6"])
...
if __name__=='__main__':
    ...
    with mirrored_strategy.scope():
        ...
        mouth_model = Conv3D(128, padding="same", kernel_size=(3, 3, 3))(mouth_model)
        mouth_model = Activation('relu')(mouth_model)
        mouth_model = Conv3D(128, padding="same", kernel_size=(3, 3, 3))(mouth_model)
        mouth_model = Activation('relu')(mouth_model)
        mouth_model = MaxPooling3D(pool_size=(3, 3, 3), padding="valid")(mouth_model)
        mouth_model = Dropout(0.25)(mouth_model)
        ...
        right_model = Conv3D(128, padding="same", kernel_size=(3, 3, 3))(right_model)
        right_model = Activation('relu')(right_model)
        right_model = Conv3D(128, padding="same", kernel_size=(3, 3, 3))(right_model)
        right_model = Activation('relu')(right_model)
        right_model = MaxPooling3D(pool_size=(3, 3, 3), padding="valid")(right_model)
        right_model = Dropout(0.25)(right_model)
    ...
    """X_train, X_test, Y_train, Y_test = train_test_split(
        righteye_train_data, train_label, test_size=0.2, random_state=43)

    history = right_model.fit(X_train, Y_train, validation_data=(X_test, Y_test), batch_size=args.batch,
                              epochs=args.epoch, verbose=1, shuffle=True)

    loss, acc = right_model.evaluate(X_test, Y_test, verbose=1)
    print('Test loss:', loss)
    print('Test accuracy:', acc)"""
