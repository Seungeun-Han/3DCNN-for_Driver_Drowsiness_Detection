...
'''================================================================================================='''
# eyemouth: em = left (1), mouth (2), right (3)
em = 2
d_dir = 'D:/dataset/NTHU-DDD-gamma-contrast/Training_Evaluation_Dataset/Training_Dataset/'
t_dir = 'D:/dataset/NTHU-DDD/Training_Evaluation_Dataset/Training_Dataset/'
u_dir = 'D:/dataset/'
'''================================================================================================='''
...
def prompt_computing(person, glass, sleepy, eyemouth, f, ovp, s, data_dir, label_dir, upper_dir, save_dir):
    ...
    #rawinput = d.Encode_input(person, glass, sleepy, eyemouth)
    rawlabel = t.Encode_target(person, glass, sleepy)
    #c3d_input = c3d.Preprocess_OnlyInput(rawinput, f, ovp, s)
    c3d_label = c3d.Preprocess_OnlyLabel(rawlabel, f, ovp, s)
    print('계산완료.')
    print('[dimension]')
...
def main():
    ...
    reserved = os.path.join('D:/dataset/')
    rlist = os.listdir(reserved)
    for x in rlist:
        dx = ''
        if x.startswith('C3D'):
            dx = x
            print(x)
        dx += '/'
        xPath = os.path.join('./Data/', dx)
        xList = os.listdir(xPath)
        for j, y in enumerate(xList):
            if y.startswith('c3d_input') and y.endswith('.npy'):
                name = y.split('_')
                print(f'\t {name[2]} {name[3]} {name[4]} {name[5]}')
    ...
    if isPrompt == False:
        while True:
            f = int(input('>>>>> 입력할 frame. 작으면 작을 수록 더 순간적인 행동을 탐지합니다. \n '
                          'Normally even number and frame <=32: \n'))
            print(f'{f} 선택함\n')
            frame = f
            if frame > 0 and frame <=32:
                break
            else:
    ...
    ## 저장 경로 생성 및 지정
    upDir = C3D_Preprocessing('./Data/')
    filePath = upDir.MakeDir_CZ3D(today, frame, overlap)
    ...
    for p in range(size+1):
        ...
        if n-1 != 2:
            ...
            for gls in range(5):
                for slp in range(4):
                    prompt_computing(person=n, glass=(gls+1), sleepy=(slp+1), eyemouth=em, f=frame, ovp=overlap, s=step, data_dir=d_dir, label_dir=t_dir, upper_dir=u_dir, save_dir=filePath)
