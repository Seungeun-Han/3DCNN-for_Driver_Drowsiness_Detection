...
def plot_history(history, result_dir, today, type, fovp, batch, epoch):
    ...
    plt.savefig(os.path.join(result_dir, today + type + fovp +
                             '_batch' + str(batch) + '_epoch' + str(epoch) + '_accuracy' + '.png'))
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, today + type + fovp +
                             '_batch' + str(batch) + '_epoch' + str(epoch) + '_loss' + '.png'))
    plt.close()


def save_history(history, result_dir, today, type, fovp, batch, epoch):
    ...
    with open(os.path.join(result_dir, today + type + fovp + '_batch' + str(batch) + '_epoch' + str(epoch) + '_result' + '.txt'), 'w') as fp:
...
if __name__=='__main__':
    ...
    parser.add_argument('--batch', type=int, default=16)          ### 수정 ###
    ...
    parser.add_argument('--type', type=str, default='gc')         # 일단 gamma, contrast 고정
    parser.add_argument('--fovp', type=str, default='_f10ovp0')    ### 수정 ###
    ...
    train_label = np.load('../Dataset/Train_Concat/Labels_f10p0_00071524_064_face.npy')
    face_train_data = np.load("../Dataset/Train_Concat/Inputs_f10p0_00071524_064_face.npy")            ### 수정 ###
    mouth_train_data = np.load("../Dataset/Train_Concat/Inputs_f10p0_00071524_mouth.npy")           ### 수정 ###
    righteye_train_data = np.load("../Dataset/Train_Concat/Inputs_f10p0_00071524_righteye.npy")     ### 수정 ###
    ...
    result_dir = args.output + today + args.type + args.fovp + '_batch' + str(args.batch) + '_epoch' + str(args.epoch) + '/'
    createDirectory(result_dir)
    
    plot_history(history, result_dir, today, args.type, args.fovp, args.batch, args.epoch)
    save_history(history, result_dir, today, args.type, args.fovp, args.batch, args.epoch)
    final_model.save_weights(os.path.join(result_dir, today + args.type + args.fovp +
                                          '_batch' + str(args.batch) + '_epoch' + str(args.epoch) + '_model' + '.h5'))